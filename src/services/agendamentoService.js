...
export const getAgendaSemanal = async (data = new Date()) => {
    try {
        ...
        const response = await axios.get(`${API_URL || _API_URL_PROD}/api/consultas/agenda`, {
...
export const criarAgendamento = async (dadosAgendamento) => {
    try {
        ...
        const response = await axios.post(`${API_URL || _API_URL_PROD}/api/consultas`, {
            agendaDisponivelId,
            petId,
            tipo,
            obs: obs || ""
...
export const getVeterinarios = async () => {
    const token = await getToken();
    const response = await axios.get(`${API_URL || _API_URL_PROD}/api/consultas/veterinarios`, {
        headers: { Authorization: `Bearer ${token}` }
    });
    return response.data;
};
...
export const getAgendaDisponivelDates = async (vetId) => {
    try {
        ...
        const response = await axios.get(`${API_URL || _API_URL_PROD}/api/consultas/horarios`, {
            params: { vet_id: vetId },
            headers: { Authorization: `Bearer ${token}` }
        });
...
export const getAgendaTutor = async () => {
    try {
        ...
        const response = await axios.get(`${API_URL || _API_URL_PROD}/api/consultas/marcadas`, {
            headers: { 
